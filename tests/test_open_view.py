...
def test_open_goal_is_shown_by_default(trivial) -> None:
    assert trivial.q() == RenderResult(
        [
            RenderRow(1, 1, "Start", True, True, True, []),
        ],
        roots={1},
    )
...
def test_closed_goal_is_not_shown_by_default(two_goals) -> None:
    assert two_goals.q() == RenderResult(
        [
            RenderRow(1, 1, "Open", True, True, True, []),
        ],
        roots={1},
    )
...
def test_closed_goal_is_shown_after_switch(two_goals) -> None:
    ...
    assert two_goals.q() == RenderResult(
        [
            RenderRow(1, 1, "Open", True, True, True, [child(2)]),
...
def test_closed_goals_are_shown_when_selected() -> None:
    ...
    assert v.q() == RenderResult(
        [
            RenderRow(1, 1, "Root", True, True, True, [child(2), child(3)]),
            RenderRow(2, 2, "closed", False, True, True, []),
            RenderRow(3, 3, "closed too", False, True, True, [child(4)]),
            RenderRow(4, 4, "closed and not selected", False, False, True, []),
        ],
        roots={1},
    ...
    assert v.q() == RenderResult(
        [
            RenderRow(1, 1, "Root", True, True, True, [child(2), child(3)]),
            RenderRow(2, 2, "closed", False, True, True, []),
            RenderRow(3, 3, "closed too", False, True, True, []),
        ],
        roots={1},
        global_opts={OPTION_SELECT: 3, OPTION_PREV_SELECT: 2},
    )
...
def test_do_not_build_fake_links_to_far_closed_goals() -> None:
    ...
    assert v.q() == RenderResult(
        [
            ...
            RenderRow(3, 3, "Top", False, False, True, []),
        ],
        roots={1, 3},
...
def test_still_show_root_when_it_is_closed_and_unselected() -> None:
    ...
    assert v.q() == RenderResult(
        [
            RenderRow(1, 1, "Hidden root", False, True, True, [child(2)]),
            RenderRow(2, 2, "Visible", False, False, True, []),
        ],
        roots={1},
...
def test_do_not_add_dangling_goals_to_old_root_on_zoom() -> None:
    ...
    assert v.q() == RenderResult(
        [
            RenderRow(
                2, 2, "Zoom root", True, False, True, [child(5)], {"Zoom": "Root goal"}
            ),
            RenderRow(4, 4, "Previous top", False, False, True, []),
            RenderRow(5, 5, "Current top", True, True, True, []),
        ],
        roots={2, 4},
