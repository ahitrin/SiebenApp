...
def test_empty_string_means_no_filter(goaltree) -> None:
    ...
    assert goaltree.q() == RenderResult(
        [
            RenderRow(1, 1, "Alpha", True, False, True, [child(2)]),
            RenderRow(2, 2, "Beta", True, False, True, [child(3)]),
...
def test_filter_by_substring(goaltree) -> None:
    ...
    assert goaltree.q() == RenderResult(
        [
            RenderRow(1, 1, "Alpha", True, False, True, [], {"Filter": "ph"}),
        ],
        roots={1},
    )
...
def test_selected_goal_must_not_be_filtered_out(selected_goaltree) -> None:
    ...
    assert selected_goaltree.q() == RenderResult(
        [
            RenderRow(2, 2, "Beta", True, False, True, [child(3)], {"Filter": "be"}),
            RenderRow(3, 3, "Gamma", True, True, True, []),
        ],
        roots={2},
...
def test_filter_is_case_insensitive(goaltree) -> None:
    ...
    assert goaltree.q() == RenderResult(
        [
            RenderRow(2, 2, "Beta", True, False, True, [], {"Filter": "eta"}),
        ],
        roots={2},
    )
