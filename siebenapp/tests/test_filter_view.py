...
def test_empty_string_means_no_filter(goaltree):
    ...
    assert goaltree.q("name,edge,select").slice("name,edge,select") == {
...
def test_filter_by_substring(goaltree):
    ...
    assert goaltree.q("name,edge,select,open,switchable").slice(
        "name,edge,select,open,switchable"
    ) == {
        1: {
            "name": "Alpha",
...
def test_zoomed_parent_goal_must_not_be_filtered_out(zoomed_goaltree):
    ...
    assert zoomed_goaltree.q("name,edge,select").slice("name,edge,select") == {
        -1: {
            "name": "Alpha",
