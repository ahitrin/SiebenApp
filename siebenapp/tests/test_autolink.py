...
def test_show_new_pseudogoal_on_autolink_event(tree_2_goals):
    ...
    assert goals.q("name,open,select,switchable").slice("name,open,select,switchable")[
        -12
    ] == {
        "name": "Autolink: 'hello'",
        "open": True,
...
def test_remove_autolink_on_close(tree_2_goals):
    ...
    assert goals.q("edge,open").slice("edge,open") == {
...
def test_remove_autolink_on_parent_delete(tree_3i_goals):
    ...
    assert goals.q("edge").slice("edge") == {1: {"edge": []}}
    assert _autolink_events(goals) == [
        ("add_autolink", 3, "test"),
        ("remove_autolink", 3),
    ]
